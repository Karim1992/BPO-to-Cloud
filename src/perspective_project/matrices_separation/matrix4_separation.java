...
public class matrix4_separation extends JFrame {
    public static JTable table_separation4;
    public static DefaultTableModel matrix4_separation_model;
    public static JButton bouton4_validation_separation;
    String task_name1, task_name2, task_name3, task_name4;
  public matrix4_separation() {
   ...
   ListModel lm = new AbstractListModel() {
      String headers[] = { task_name1, task_name2, task_name3, task_name3 };
      public int getSize() {
    ...
    matrix4_separation_model = new DefaultTableModel(lm.getSize(), 4) {
    ...
    */
    ...
    table_separation4 = new JTable(matrix4_separation_model);
    ...
    String titres_colonnes[] = { task_name1, task_name2, task_name3, task_name4 };
    matrix4_separation_model.setColumnIdentifiers(titres_colonnes);
    //remplir la diagonale avec des 0 puisque une tâche ne peut pas être séparée d'elle même 
    matrix4_separation_model.setValueAt(0, 0, 0);
    matrix4_separation_model.setValueAt(0, 1, 1);
    matrix4_separation_model.setValueAt(0, 2, 2);
    matrix4_separation_model.setValueAt(0, 3, 3);
    
    //remplir la matrice avec des 0 (par defaut il n'y pas de separation de tâches)
for(int x=0; x<=3; x++){
  for(int y=0; y<=3; y++){
    matrix4_separation_model.setValueAt(0, x, y);
    ...
    table_separation4.setRowHeight(20);
    rowHeader.setFixedCellHeight(table_separation4.getRowHeight());
    //    + table.getRowMargin());
    //                           + table.getIntercellSpacing().height);
    rowHeader.setCellRenderer(new RowHeaderRenderer(table_separation4));

    JScrollPane scroll = new JScrollPane(table_separation4);
   scroll.setRowHeaderView(rowHeader);
    this.setLayout(new BorderLayout());
    this.add(scroll, BorderLayout.CENTER);
    
   bouton4_validation_separation = new JButton("Validate task separation");
     JButton bouton_remplissage = new JButton("Complete matrix");
       JPanel panel = new JPanel();
       panel.add(bouton4_validation_separation, BorderLayout.EAST);
   ...
   bouton_remplissage.addActionListener(new ActionListener() {    // définir la réaction du bouton en dessous de la matrice de separation
                    public void actionPerformed(ActionEvent evt) {
                      for(int i=0; i<=3; i++){
                          for(int j=0; j<=3; j++){
                           if(i < j){   
                               int value = Integer.valueOf(matrix4_separation_model.getValueAt(i, j).toString());
                               matrix4_separation_model.setValueAt(value, j, i);
                           }
                          }
                      }   
...
/*   DefaultTableCellRenderer renderer = new DefaultTableCellRenderer();
  ...
  */  
    table_separation4.setDefaultRenderer(Object.class, new ColorRenderer());
